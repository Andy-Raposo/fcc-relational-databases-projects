...
echo "$($PSQL "SELECT name FROM games AS g INNER JOIN teams AS t ON g.winner_id = t.team_id WHERE (round='Eighth-Final' AND year=2014) UNION SELECT name FROM games AS g INNER JOIN teams AS t ON g.opponent_id = t.team_id WHERE (round='Eighth-Final' AND year=2014) ORDER BY name")"
